...
final class APICaller {
    ...
    public func getRecommendations(genres : Set<String> , completion : @escaping ((Result<RecommendationsResponse  , Error>)) -> Void){
        ...
        createRequest(with:URL(string: Constants.baseAPIURL + "/recommendations?limit=4&seed_genres=\(seeds)"), type: .GET) { request in
            let task = URLSession.shared.dataTask(with: request){data , _ , error in
...
public func getRecommendedGenres(completion : @escaping ((Result<RecommendedGenresResponse  , Error>)) -> Void){
    ...
    public func getPlaylistDetails (for playlist : Playlist , completion : @escaping(Result<PlaylistDetialsResponse , Error>) -> Void ){
        createRequest(
            ...
            complition: { request in
                ...
                let task = URLSession.shared.dataTask(with: request){ data , _ , error in
                    ...
                    do {
                       ...
                       let result = try JSONDecoder().decode(PlaylistDetialsResponse.self , from : data)
                      // let result = try JSONSerialization.jsonObject(with: data, options: .allowFragments)
    ...
    public func getCategories ( completion : @escaping(Result<[Category] , Error>) -> Void ){
        createRequest(
            with: URL(string: Constants.baseAPIURL + "/browse/categories?limit=10"),
            type: .GET,
            complition: { request in
